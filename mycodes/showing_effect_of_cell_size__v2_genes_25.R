...
# load("./NAc_rse_gene_withCompEsts.rda")
# e <- new.env()
# load(file.path("~", "cell_type_data", "NAc_rse_gene_withCompEsts_update.rda"), envir = e)
# 
# identical(e$rse_gene$BrNum, rse_gene$BrNum)
# identical(e$rse_gene$NeuN_pos_DNAm, rse_gene$NeuN_pos_DNAm)
...
# Plot
{
  ...
  if(!file.exists(file.path(".", "model", "top_25"))){
    dir.create(file.path(".", "model", "top_25"))
  }
  ...
  p_save <- ggplot(data = out_pd[type %in%set ], aes(x = est, y = `Houseman DNAm-based`, color = type, shape = type)) + 
    ...
    annotation_custom(gridExtra::tableGrob(mytable[set_idx,], rows = NULL), xmin=0.2, xmax=.2, ymin=.35, ymax=.3) +
  ...
  ggsave(file.path(".", "model", "top_25", "bias_correction.png"), plot = p_save, dpi = "retina", width = 40, height = 30, units = "cm")
  ...
  p_save <- ggplot(data = out_pd, aes(x = est, y = `Houseman DNAm-based`, color = type, shape = type)) + 
    ...
    scale_color_manual(values = p_save_cols, name = "") +
    scale_shape_manual(values = p_save_shape, name = "" ) +
    annotation_custom(gridExtra::tableGrob(mytable, rows = NULL), xmin=0.24, xmax=.24, ymin=.38, ymax=.38) +
    transparent_legend + remove_grid +
    ggtitle("Relationship between Houseman Methylation and cell-type proportion estimates\n(Neurons only)") +
  ...
  ggsave(file.path(".", "model", "top_25", "bias_correction_v2.png"), plot = p_save, dpi = "retina", width = 60, height = 50, units = "cm")
  ...
  p_save <- ggplot(data = out_pd[!type %in% set ], aes(x = est, y = `Houseman DNAm-based`, color = type, shape = type)) + 
    ...
    theme(plot.title = element_text(size = 12, face = "bold", hjust = 0.5),
          ...
          legend.position = c(0.85, 0.10),
          legend.title = element_text(face="bold"))
  ggsave(file.path(".", "model", "top_25", "bias_correction_v3_Darmanis_ref.png"), plot = p_save, dpi = "retina", width = 40, height = 30, units = "cm")
  ...
  p_save <- ggplot(data = out_pd[type %in% set], aes(x = est, y = `Houseman DNAm-based`, color = type, shape = type)) + 
    ...
    theme(plot.title = element_text(size = 12, face = "bold", hjust = 0.5),
          ...
          legend.position = c(0.85, 0.10),
          legend.title = element_text(face="bold"))
  ggsave(file.path(".", "model", "top_25", "bias_correction_v3_nac_ref.png"), plot = p_save, dpi = "retina", width = 40, height = 30, units = "cm")
  ...
  sapply(1:n_distinct(out_pd$type), function(i){
    p_save <- ggplot(data = out_pd[type == t[i], ], aes(x = est, y = `Houseman DNAm-based`, color = type)) + 
      ...
      theme(plot.title = element_text(size = 12, face = "bold", hjust = 0.5),
            ...
            legend.title = element_text(face="bold"))
    ggsave(file.path(".", "model", "top_25", paste0("bias_correction_type", i, ".png")), plot = p_save, dpi = "retina", width = 30, height = 20, units = "cm")
  })
  ...
  p_save <- ggplot(data = out_pd_v3, aes(x = est, y = `Houseman DNAm-based`, color = type, shape = type)) + 
    ...
    theme(plot.title = element_text(size = 14, face = "bold", hjust = 0.5),
          ...
          legend.title = element_text(face="bold"), 
          legend.text = element_text(size = 12, face="bold"))
  ggsave(file.path(".", "model", "top_25", "bias_correction_new_darmanis_ref.png"), plot = p_save, dpi = "retina", width = 60, height = 50, units = "cm")
  ...
  p_save <- ggplot(data = out_pd_v4, aes(x = est, y = `Houseman DNAm-based`, color = type, shape = type)) + 
    ...
    theme(plot.title = element_text(size = 14, face = "bold", hjust = 0.5),
          ...
          legend.title = element_text(face="bold"), 
          legend.text = element_text(size = 12, face="bold"))
  ggsave(file.path(".", "model", "top_25", "bias_correction_new_NAc_ref.png"), plot = p_save, dpi = "retina", width = 60, height = 50, units = "cm")
}
...
fwrite(rbind(mytable_nac, mytable_dar), 
       file = file.path("cell_type_data", "top_25.txt"))
